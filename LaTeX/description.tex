...
read-only instead of write-only. This makes it so that new images can only be rendered to areas not 
written to in the first pass. Before finally outlining the image using GL\_Line mode and creating 
the model seen infigure 8. A much less detailed contour, but without any of the artifacts of 

...
The second implementation used the stencil buffer rather than offsetting the outline. To do this, we needed to render in the reverse order of our first attempt. This is as 
the stencil buffer first needs to be told where it can to write to. The stencil buffer  
first needs to be enabled and cleared. Next where on the screen information should be shown 

is stored in the matrix before rendering the base model. 

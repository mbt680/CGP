...
the model a realistic 3D shape. Specular light concentrates light in one direction, providing 
...
Rim lighting is also added to provide additional highlights and increase the cartoon appearance 
...
Lighting is implemented in the fragment shader, rather than in the vertex shader, decreasing 
...
source. A code snippet of our implementation is provided in \autoref{fig-phong-lighting}.
...
\begin{figure}[h]
    ...
    \end{lstlisting}
    \caption{Fragment shader code for Phong and rim lighting}
...
While this was happening, suggestive contours would be calculated inside of the shaders and stored in a matrix, 
...
through cel shading so photo realistic versions are rendered in a consistent toon style.
...
Using a photo-realistic image transformed through cel shading as the new texture can produce interesting 
